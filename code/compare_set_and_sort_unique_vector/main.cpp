...
void test() {
  ...
  {
    TM::BlockMeasurement measure("sort and unique vector");
    ...
    std::cout << "vector size is " << sort_unique_vector.size() << std::endl;
  }

  {
    TM::BlockMeasurement measure("set");
    std::set<size_t> set;
    // set
    for (size_t i = 0; i < start_values.size(); ++i) {
      const auto cnt = cnts[i];
    ...
    std::cout << "set size is " << set.size() << std::endl;
  }
}
